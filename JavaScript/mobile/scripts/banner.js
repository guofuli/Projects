...
window.onload=function(){
    ...
    btn_1.addEventListener( 'touchstart',function(){
        clearInterval(t);
        i--;
        move();
    });
    btn_1.addEventListener( 'touchend',function(){
    ...
    btn_2.addEventListener( 'touchstart',function(){
        clearInterval(t);
        i++;
        move();
    });
     btn_2.addEventListener( 'touchend',function(){
        t=setInterval(function(){
            i++;
            move();
        },3000);
    ...
    function move(){
        ...
        if(i==-1){
            img_list.style.left=-(size-1)*device_width+"px";
            i=size-2;
        }
        moveElement("img-move",-i*device_width,0,10);
        if(i==size-1){
            for (var m=0;m<num_li.length;m++){
                (function (m){
                    if(m==0){
                        num_li[m].setAttribute("class","on");
                    }else {num_li[m].setAttribute("class", "")};
                })(m);
            }
        }else{
            ...
            for (var z=0;z<num_li.length;z++){
                (function (z){
                    ...
                    }else {num_li[z].setAttribute("class", "")};
                })(z);
            }
        }
    }
